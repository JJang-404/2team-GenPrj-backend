...
```ts
export const API_BASE_URL = 'https://gen-proj.duckdns.org/addhelper';
```
...
```ts
const API_BASE_URL = 'https://gen-proj.duckdns.org/addhelper';
...
```ts
const blob = await generateImage('맛있는 크루아상과 커피');
...
changeimage 호출 전에는 업로드한 이미지를 base64 문자열로 바꿔야 합니다.
